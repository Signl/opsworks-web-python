...
define :django_configure do
  ...
  if deploy[:deploy_to] && (node[:deploy][application]["initially_deployed"] || ::File.exist?(deploy[:deploy_to]))
    ...
    if gunicorn["enabled"]
      ...
      my_gunicorn_cfg = ::File.join(deploy[:deploy_to], 'shared', 'gunicorn-config.py')
      ...
      gunicorn_command = "#{base_command} -c #{my_gunicorn_cfg}"# --certfile #{gunicorn_cert} --keyfile #{gunicorn_key}"
